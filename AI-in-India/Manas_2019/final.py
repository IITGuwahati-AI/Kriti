...
''' Manas Hostel - iitg.ai submission - Training a CNN to identify patients that have Glaucoma
...
''' 
The variables store the path of the training data and validation data. 
The training will be done for 100 epochs and the images will be sent in batches of 16.
'''
...
#Saved the dimensions of the image
...
#using batch normalization to adjust and scale the activation
block1 = BatchNormalization(name='norm_0')(input)



# Block 1 of neural network
...
#we use the categorical crossentropy loss function and the Adam optimizer
model.compile(loss="categorical_crossentropy", optimizer=optimizers.Adam(lr=0.01, beta_1=0.9, beta_2=0.999, epsilon=None, decay=0.0, amsgrad=False), metrics=["accuracy"])



# Initiate the train and test generators with data Augumentation to diversify the dataset
